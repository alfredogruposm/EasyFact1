            ...
            if (result == DialogResult.Yes)
            {
                if (comboBox2.SelectedIndex != -1)
                {
                    if (dataGridView1.CurrentRow.Cells[2].Value.ToString() == "")
                    {
                        ...
                        string ID = Convert.ToInt32(dataGridView1.CurrentRow.Cells["ID"].Value);
                        string cadenaXML = "<?xml version=" + "1.0 " + " encoding=" + "UTF-8" + "? >" + xml;
                        string ruta = "C:\\Facturas CFDI\\FacturasXMLSinSello\\" + ID + ".xml";
                        File.WriteAllText(ruta, cadenaXML);
                        ...
                        string[] respuesta = new string[4];
                        EquimarFac.DAO.TimbradoFelWebService2015 timbrado = new EquimarFac.DAO.TimbradoFelWebService2015();
                        timbrado.cadenaXML = ruta;
                        ...
                        if (respuesta[0] == "")
                        {
                            ...
                            File.WriteAllText("C:\\Facturas CFDI\\FacturasXML\\"+ nodotexto2 + ".xml", resultado1);
